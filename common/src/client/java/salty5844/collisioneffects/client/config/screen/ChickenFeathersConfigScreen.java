...
public final class ChickenFeathersConfigScreen extends Screen {
	...
	public ChickenFeathersConfigScreen(Screen parent) {
		super(Component.literal("Chicken"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		this.addRenderableWidget(Button.builder(Component.literal("Reset"), button -> {
			config.resetChickenFeathersDefaults();
			this.clearWidgets();
			this.init();
		...
		addToggleRow(
			"Hit Chicken Feathers",
			config.isEnabled("chicken_hit_feathers"),
			contentX,
			48,
			value -> config.setEnabled("chicken_hit_feathers", value)
		);

		addToggleRow(
			"Touch Chicken Feathers",
			config.isEnabled("chicken_touch_feathers"),
			contentX,
			72,
			value -> config.setEnabled("chicken_touch_feathers", value)
		);
