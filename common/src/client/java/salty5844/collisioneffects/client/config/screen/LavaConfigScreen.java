...
public final class LavaConfigScreen extends Screen {
	...
	public LavaConfigScreen(Screen parent) {
		super(Component.literal("Lava"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		this.addRenderableWidget(Button.builder(Component.literal("Reset"), button -> {
			config.setEnabled("lava_splatters", true);
			this.clearWidgets();
			this.init();
		...
		addToggleRow(
			"Lava Splatters",
			config.isEnabled("lava_splatters"),
			contentX,
			rowY,
			value -> config.setEnabled("lava_splatters", value)
		);
