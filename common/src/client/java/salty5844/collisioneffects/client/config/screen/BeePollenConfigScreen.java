...
public final class BeePollenConfigScreen extends Screen {
	...
	public BeePollenConfigScreen(Screen parent) {
		super(Component.literal("Bee"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		addToggleRow(
			"Damaged by Pollinated Bee Pollen",
			config.isEnabled("bee_pollen"),
			contentX,
			48,
			value -> config.setEnabled("bee_pollen", value)
		);

		addToggleRow(
			"Hit Pollinated Bee Pollen",
			config.isEnabled("bee_pollen_hit"),
			contentX,
			72,
			value -> config.setEnabled("bee_pollen_hit", value)
		);

		addToggleRow(
			"Touch Pollinated Bee Pollen",
			config.isEnabled("bee_pollen_touch"),
			contentX,
			96,
			value -> config.setEnabled("bee_pollen_touch", value)
		);
