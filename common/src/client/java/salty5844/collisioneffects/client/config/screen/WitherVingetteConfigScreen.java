...
public final class WitherVingetteConfigScreen extends Screen {
	...
	public WitherVingetteConfigScreen(Screen parent) {
		super(Component.literal("Wither Vignette"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		String titleText = "Wither Vignette";
		...
		this.addRenderableWidget(Button.builder(Component.literal("Reset"), button -> {
			config.resetWitherVingetteDefaults();
			this.clearWidgets();
			this.init();
		...
		addToggleRow(
			"Wither Vignette",
			config.isEnabled("wither_vingette"),
			contentX,
			rowY,
			value -> config.setEnabled("wither_vingette", value)
		);
