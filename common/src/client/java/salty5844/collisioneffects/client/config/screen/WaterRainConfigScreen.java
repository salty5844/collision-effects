...
public final class WaterRainConfigScreen extends Screen {
	...
	public WaterRainConfigScreen(Screen parent) {
		super(Component.literal("Rain/Water"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		addToggleRow(
			"Rain Drops",
			config.isEnabled("rain_droplets"),
			contentX,
			rowY,
			value -> config.setEnabled("rain_droplets", value)
		);
		rowY += ROW_HEIGHT;

		addToggleRow(
			"Water Splashes",
			config.isEnabled("water_droplets"),
			contentX,
			rowY,
			value -> config.setEnabled("water_droplets", value)
		);
