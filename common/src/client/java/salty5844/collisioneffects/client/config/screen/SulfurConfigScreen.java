...
public final class SulfurConfigScreen extends Screen {
	...
	public SulfurConfigScreen(Screen parent) {
		super(Component.literal("Sulfur Cube"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		addToggleRow(
			"Hit Sulfur Cube Splatters",
			config.isEnabled("sulfur_hit_splatters"),
			contentX,
			rowY,
			value -> config.setEnabled("sulfur_hit_splatters", value)
		);
		rowY += ROW_HEIGHT;

		addToggleRow(
			"Touch Sulfur Cube Splatters",
			config.isEnabled("sulfur_touch_splatters"),
			contentX,
			rowY,
			value -> config.setEnabled("sulfur_touch_splatters", value)
		);
